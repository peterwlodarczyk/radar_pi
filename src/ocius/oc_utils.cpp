...
string MakeLocalTimeStamp()
{
	...
	sprintf(achTemp, "%04d-%02d-%02d %02d-%02d-%02d-%03ld", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour,
		tm->tm_min, tm->tm_sec, msecs);
#endif
	return achTemp;
}
...
std::vector<uint8_t> JpegAppendComment(const std::vector<uint8_t>& input, const string& timestamp, const string& camera)
{
	...
	if (input.size() < 2 || input[input.size() - 2] != achEOI[0] || input[input.size() - 1] != achEOI[1])
		return output;
